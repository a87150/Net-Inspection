...
5. **手动执行分析**：先获取新日志，再合并当前来源、目录和日期范围内已入库的有效日志，自动执行规则分析、保存结果及生成告警；没有新文件也会重新分析范围内已有日志。任务详情分别展示获取、分析两阶段，可进入分析结果或结束尚未完成的分析。

手动执行会冻结点击时的来源、日期范围、已有日志、规则、人员范围与告警策略，新获取的有效日志再合并到分析任务。已归档日志直接复用数据库证据，不重复下载或导入；每次分析生成新记录，保留历史结果。定时任务保持增量处理，仅分析本次获取的有效日志。同一配置的分析阶段未结束时，重复手动提交会被拦截。

如果获取失败但已有符合范围的日志，仍继续分析已有证据，获取阶段保留失败/部分成功状态；这不表示采集到了最新数据。范围内完全没有有效日志时明确提示未执行分析，不会用空获取结果冒充分析完成。
...
“分析规则与阈值”中的**软件分析模式**提供两项：白名单模式只读取 `WHITELIST` 与 `SPECIAL_WHITELIST`，已安装但不在允许范围的软件报问题；黑名单模式只读取 `BLACKLIST.keywords`，命中才报问题，未列出的软件不报。匹配沿用原有不区分大小写的名称关键词包含规则，白名单不是必装软件清单。两种模式不交叉使用，即同一个软件同时在两个列表时，仅以所选模式为准。

无需更改或重新上传策略文件；未配置策略仍只展示清单。新增配置及升级后的现有配置默认白名单，可自行切换。模式和文件内容在入队时冻结，获取后的分析子任务沿用同一快照；旧的无模式任务保持原有黑名单优先、再检查白名单的行为，历史结果不重算。升级需应用 `0050_pc_software_policy_mode` 并重启 Web/Worker，终端采集器无需更新。
...
新任务的人员为主模式只查询在职人员，离职人员及其日志不进入该模式的分析结果、缺日志统计和总结；日志为主模式仍保留原有日志范围及人员上下文。人员在任务入队后离职不改写该任务，旧任务不按当前在职状态重算。

PC 保留独立的“问题等级设置”：使用率、温度、磁盘、连续开机时长、补丁与 Defender 间隔可覆盖分析配置阈值，留空沿用原值。只对新入队任务生效。提示、业务异常和执行失败分别保存；调整业务等级不会把执行失败变为成功。

- 在“PC 日志来源与分析配置 → 人员匹配方式”选择人员为主或日志为主，默认日志为主。人员为主以入队时的在职人员（`is_active=True`）匹配当前任务日志：排除离职人员，保留在职无日志人员，并在已结束任务的统计中计为“人员缺少日志”警告，不伪造分析记录。日志为主相当于人员 RIGHT JOIN 日志：保留找不到人员的日志。
- 工号优先（支持域前缀/UPN），没有工号时使用唯一姓名或工号命名的计算机匹配；姓名比较时双方忽略第一个半角 `-` 及其后的说明（如 `测试乙-示例客服部门` 按 `测试乙` 匹配），原始姓名和工号不修改，参考对应检查也应用此规则；同名不猜测，有明确工号但未匹配时不以姓名覆盖。人员名册在任务入队时冻结（获取任务与分析子任务共用），列表与导出共用同一匹配结果。
